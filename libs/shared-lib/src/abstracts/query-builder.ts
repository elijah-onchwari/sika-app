            ...
            new Brackets((bck: WhereExpressionBuilder) => {
              switch (type) {
                case 'between':
                  ...
                  bck.andWhere(
                    `"${qb.alias}"."${column}" ${type} '${start}' AND '${end}'`
                  );
                  break;
                case 'ilike':
                ...
                case 'like':
                  bck.andWhere(
                    `LOWER("${qb.alias}"."${column}") ${type} LOWER(:${column})`,
                    {
                      [column]: value,
                    }
                  );
          ...
          if (entry instanceof FindOperator) {
            ...
            switch (type) {
              case 'between':
                ...
                const [start, end] = value;
                bck.andWhere(
                  `"${alias}"."${column}" ${type} '${start}' AND '${end}'`
                );
                break;
              case 'ilike':
              ...
              case 'like':
                bck.andWhere(
                  `LOWER("${alias}"."${column}") ${type} LOWER(:${column})`,
                  {
                    [column]: value,
                  }
                );
